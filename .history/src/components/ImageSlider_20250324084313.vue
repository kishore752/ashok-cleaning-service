...
const nextSlide = () => {
  if (currentIndex.value === images.length - 1) {
    // When at the last image, set a flag to indicate we're transitioning to first
    isTransitioning.value = true
    // First move to a clone of the first image
    currentIndex.value = images.length
    // After the transition, move to the first image
    setTimeout(() => {
...
const prevSlide = () => {
  if (currentIndex.value === 0) {
    // When at the first image, set a flag to indicate we're transitioning to last
    isTransitioning.value = true
    // First move to a clone of the last image
    currentIndex.value = -1
    // After the transition, move to the last image
    setTimeout(() => {
